...
class ProgressBar {
public:
  ProgressBar(int total_iterations, int bar_width) : total_iterations_(total_iterations), bar_width_(bar_width) {
    start_time_ = std::chrono::high_resolution_clock::now();
  }

  void Update(int progress) {
    if (!complete_ && progress >= next_report_iteration_)
      Print(progress);
  }
  ...
  void Print(int progress) {
    ...
    std::cout << " (" << std::setw(2) << std::setfill('0') << static_cast<int>(elapsed_time.count()) / 60;
    std::cout << ":" << std::setw(2) << std::setfill('0') << static_cast<int>(elapsed_time.count()) % 60 << ")";
